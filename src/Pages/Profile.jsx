...
if (res.status === 401 || res.status === 403) { alert("Session expired. Please login again"); await logOut(); window.location.href = "/login"; return; }
      Swal.fire({
        icon: "success",
    ...
    <form onSubmit={handleUpdate} className="space-y-3 sm:space-y-4">
      ...
      <button
        ...
        className={`w-full py-2.5 sm:py-3 text-sm sm:text-base rounded-lg text-white bg-(--primary) transition ${
          uploading || updating ? "opacity-50 cursor-not-allowed" : "hover:opacity-90"
        }`}
      >
        {uploading
          ? "Uploading Image..."
